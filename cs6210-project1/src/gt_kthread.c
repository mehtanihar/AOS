...
extern void gtthread_app_init(int sched_type)
{
	...
	gt_spin_lock(&ksched_info->ksched_lock);
	ksched_info->sched_type = sched_type;
	gt_spin_unlock(&ksched_info->ksched_lock);

	/* Initialize shared schedule information (spinlocks) */
	...
	kthread_init(k_ctx_main);//CPU Binding
...
yield_again:
		...
		if(init_done < num_cpus){
			/* Kthreads haven't all been initialized */
